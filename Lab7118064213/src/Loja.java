...
public class Loja {
    ...
    public Integer quantidadeDeItens(){
        return quantidadeEmEstoquePorItem.values()
                .stream()
    ...
    public String receberPedido(Vendavel item, int quantidade, Usuario usuario)
        ...
        quantidadeEmEstoquePorItem.put(item, quantidadeEmEstoquePorItem.get(item) - quantidade);
    ...
    public void incluirItemComQuantidade(Vendavel vendavel, Integer quantidade){
        if (buscarItem(vendavel.getId()) != null) {
            // produto já existe no catálogo -- nada a fazer
            return;
        }
